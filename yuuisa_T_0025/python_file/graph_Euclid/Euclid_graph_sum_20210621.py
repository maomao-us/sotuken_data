...
def Euclid_graph():
    for a in range(8):
        for z in range(4):
            ...
            # file_mean = "Euclid/mean_Euclid/" + name[a] + "/" + name[a] + "_Euclid_" +str(z + 2) + ".csv"

            df_sum = pd .read_csv(file_sum, header=None, engine = "python")
            # df_mean = pd.read_csv(file_mean, header=None, engine = "python")

            df_sum_row = sorted(df_sum.values[a])
            df_sum_row = [x for x in df_sum_row if math.isnan(x) == False]

            for b in range(len(df_sum_row)):
                if b == 0:
                    x[a].append(df_sum_row[b])
                    y[a].append(1)
                elif 1 <= b and df_sum_row[b] != x[a][len(x[a]) - 1]:
                    x[a].append(df_sum_row[b])
                    y[a].append(1)

            plt.bar(x[a], y[a], label = label_list[a], width = 1, color = color_list[a])
            
            df_sum_row = df_sum.values[8]
            df_sum_row = [x for x in df_sum_row if math.isnan(x) == False]

            for b in range(len(df_sum_row)):
                if b != a:
                    x[b].append(df_sum_row[b])
                    y[b].append(1)
            ...
            plt.legend()
            # plt.show()
            fig.savefig("graph_Euclid/sum_Euclid/" + name[a] + "/" + label_list[a] + "_" +  name[a] + "_sum_" + str(z + 2) + ".png")
            plt.close()
