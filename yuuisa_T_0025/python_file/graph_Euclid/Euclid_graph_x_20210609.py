...
color_list =["k", "b", "g", "c", "m", "y", "r", '#f781bf']
x1 = []
x2 = []
x3 = []
x4 = []
...
def Euclid_graph_ev():
    for a in range(0, 1):
        for z in range(1, 2):
            ...
            file = "Threshold/" + name[a] + "/" + "Threshold_" + lx[z] + ".csv"
            df = pd .read_csv(file, header=None, engine = "python")

            for b in range(len(df) - 1):
                print(name[b])
                # if a != b:
                ...
                print(x[b])
                print(y[b])
                plt.bar(x[b], y[b], width = 0.5, color = color_list[b], label = label_list[b])
            plt.yticks([])
            plt.xlim(0, 50)
    ...
    for a in range(8):
        for z in range(4):
            ...
            for b in range(len(df)):
                ...
                print(x[b])
                print(y[b])
                plt.bar(x[b], y[b], label = label_list[b], width = 0.5, color = color_list[b])
            plt.legend()
            plt.show()
