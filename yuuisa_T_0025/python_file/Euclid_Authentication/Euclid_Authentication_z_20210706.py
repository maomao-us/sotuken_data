...
def Euclid():
    for a in range(0, 8):
        ...
        for z in range(0, 4):
            ...
            file_test = "Threshold/" + name[a] + "/" + "test_" + lx[z] + ".csv"
            df_test = pd.read_csv(file_test, header=None, engine = "python")

            threshold = "Threshold/" + name[a] + "/" + "Threshold_" + lx[z] + ".csv"
            ...
            if z == 0:
                with open("Threshold/Authentication/z/" + name[a] + ".csv", "w", newline = "") as f:
                    writer = csv.writer(f)
                    writer.writerow(Authentication)
            else:
                with open("Threshold/Authentication/z/" + name[a] + ".csv", "a", newline = "") as f:
                    writer = csv.writer(f)
                    writer.writerow(Authentication)
Euclid()