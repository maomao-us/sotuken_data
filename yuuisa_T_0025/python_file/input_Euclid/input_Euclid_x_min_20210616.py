...
def MIN_EV(f):
    ...
    for i in range(len(ev)):
        if i % 2 == 0:
            min_ev.append(ev[i])
            min_step.append(step_list1[i])


    for i in range(len(min_step) - 1):
        step_list2.append(min_step[i + 1] - min_step[i])
    ...
    if len(min_ev) > 1:
        ...
        print(min_step)
        print(step_list2)
    return min_ev, min_step, step_list2
...
for w in range(0, 8):
    ...
    for z in range(0, 3):
        ...
        for i in range(10, 30):
            ...
            if len(ev) == (z + 2):
                if i % 2 == 0 and i == 10:
                    with open("sotuken_data_train/data/" + name[w] + "/" + name[w] + "_train_min_" + lx[z] + ".csv", "w", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev)
                    with open("sotuken_data_train/step/" + name[w] + "/" + name[w] + "_train_min_" + lx[z] + ".csv", "w", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev_step2)
                elif i % 2 == 0 and 10 < i:
                    with open("sotuken_data_train/data/" + name[w] + "/" + name[w] + "_train_min_" + lx[z] + ".csv", "a", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev)
                    with open("sotuken_data_train/step/" + name[w] + "/" + name[w] + "_train_min_" + lx[z] + ".csv", "a", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev_step2)
                elif i % 2 ==1 and i == 11:
                    with open("sotuken_data_test/data/" + name[w] + "/" + name[w] + "_test_min_" + lx[z] + ".csv", "w", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev)
                    with open("sotuken_data_test/step/" + name[w] + "/" + name[w] + "_test_min_" + lx[z] + ".csv", "w", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev_step2)
                elif i % 2 ==1 and 11 < i:
                    with open("sotuken_data_test/data/" + name[w] + "/" + name[w] + "_test_min_" + lx[z] + ".csv", "a", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev)
                    with open("sotuken_data_test/step/" + name[w] + "/" + name[w] + "_test_min_" + lx[z] + ".csv", "a", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(ev_step2)

        df_train_ev = pd.read_csv("sotuken_data_train/data/" + name[w] + "/" + name[w] + "_train_min_" + lx[z] + ".csv", header=None, engine = "python")
        df_train_step = pd.read_csv("sotuken_data_train/step/" + name[w] + "/" + name[w] + "_train_min_" + lx[z] + ".csv", header=None, engine = "python")
        ...
        df_train_ev.to_csv("sotuken_data_train/data/" + name[w] + "/" + name[w] + "_train_min_" + lx[z] + ".csv", header=None, index = False)
        df_train_step.to_csv("sotuken_data_train/step/" + name[w] + "/" + name[w] + "_train_min_" + lx[z] + ".csv", header=None, index = False)
