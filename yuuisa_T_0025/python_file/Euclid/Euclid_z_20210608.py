...
def Euclid():
    for a in range(8):
        ...
        for z in range(0,4):
            print(name[a])
            file_train_ev = "train_data/data/" + name[a] + "/" + name[a] + "_train_" + lx[z] + ".csv"
            file_train_step = "train_data/step/" + name[a] + "/" + name[a] + "_train_" + lx[z] + ".csv"
            ...
            for b in range(8):
                file_test_ev = "sotuken_data_test/data/" + name[b] + "/" + name[b] + "_test_" + lx[z] + ".csv"
                file_test_step = "sotuken_data_test/step/" + name[b] + "/" + name[b] + "_test_" + lx[z] + ".csv"
                ...
                if b == 0:
                    with open("Euclid/data/" + name[a] + "/" + name[a] + "_Euclid_" + lx[z] + ".csv", "w", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(data_Euclid_ev)
                    with open("Euclid/step/" + name[a] + "/" + name[a] + "_Euclid_" + lx[z] + ".csv", "w", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(data_Euclid_step)
                    with open("Euclid/Euclid/" + name[a] + "/" + name[a] + "_Euclid_" + lx[z] + ".csv", "w", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(data_Euclid)
                else:
                    with open("Euclid/data/" + name[a] + "/" + name[a] + "_Euclid_" + lx[z] + ".csv", "a", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(data_Euclid_ev)
                    with open("Euclid/step/" + name[a] + "/" + name[a] + "_Euclid_" + lx[z] + ".csv", "a", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(data_Euclid_step)
                    with open("Euclid/Euclid/" + name[a] + "/" + name[a] + "_Euclid_" + lx[z] + ".csv", "a", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(data_Euclid)
