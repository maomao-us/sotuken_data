        ...
        for z in range(4):
            ...
            for b in range(len(df_x) - 1):
                for c in range(len(df_x.columns)):
                    sum_data.append(int((df_x.values[b, c] + df_z.values[b, c]) / 2))
                
                min_data.append(min(sum_data))

                if b == 0:
                    with open("Euclid/x_z_Euclid/" + name[a] + "/" + name[a] + "_Euclid_" + str(z + 2) + ".csv", "w", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(sum_data)
                
                elif 0 < b:
                    with open("Euclid/x_z_Euclid/" + name[a] + "/" + name[a] + "_Euclid_" + str(z + 2) + ".csv", "a", newline = "") as f:
                        writer = csv.writer(f)
                        writer.writerow(sum_data)
