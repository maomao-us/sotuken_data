...
def kentei_train():
    for a in range(8):
        for b in range(0,4):
            print(name[a])
            file_name1 = "sotuken_data_train/data/" + name[a] + "/" + name[a] + "_train_min_" + lx[b] + ".csv"
            file_name2 = "sotuken_data_train/step/" + name[a] + "/" + name[a] + "_train_min_" + lx[b] + ".csv"
            ...
            with open("train_data/data/" + name[a] + "/" + name[a] + "_train_min_" + lx[b] + ".csv", "w", newline = "") as f:
                writer = csv.writer(f)
                writer.writerow(mean_ev)
                writer.writerow(var_ev)
            with open("train_data/step/" + name[a] + "/" + name[a] + "_train_min_" + lx[b] + ".csv", "w", newline = "") as f:
                writer = csv.writer(f)
